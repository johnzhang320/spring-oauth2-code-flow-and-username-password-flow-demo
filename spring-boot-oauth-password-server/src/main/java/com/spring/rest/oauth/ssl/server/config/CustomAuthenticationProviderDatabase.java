...
public class CustomAuthenticationProviderDatabase implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    	...
    	String password_seeds = authentication.getCredentials().toString();
     
        int seperate = password_seeds.indexOf("|");
        
        String password = password_seeds.substring(0,seperate);
        
        String seedStr4IV = password_seeds.substring(seperate+1);
        
        EncryptionUtils handler=new EncryptionUtils(seedStr4IV);
        
        
        String username = authentication.getName();
        
        try {
        	username = handler.decrypt(username);
        	password = handler.decrypt(password);
        } catch (Exception e) {
        	logger.error ( "Error while decrypt the username and password");
